...
import { deleteFavWodService } from "../../services/wod.services";
...
function FavWods() {
  ...
  if (isFetching === true) {
    return <h3>...Loading the list of favorites wods...</h3>;
  }
        ...
        {favWodsList.map((eachWod) => {
          return (
            <div id="each-favWod" key={eachWod._id}>
              <Link to={`/${eachWod._id}/details`} id="wods-link">
                {eachWod.name}
              </Link>
              <Link to={`/${eachWod._id}/details`}>
                <button id="see-wod">See wod</button>
              </Link>

              <button
                onClick={() => deleteFavWod(eachWod._id)}
                id="delete-favWod"
              >
                Delete
              </button>
